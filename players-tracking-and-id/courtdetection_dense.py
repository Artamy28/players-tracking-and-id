...
def firstdetection(frame):
    ...
    # points = [None, None, None, None, None, None, (210, 362), None, (1887, 272), None, (1386, 557), None, (1520, 750), None]
...
def nextdetection(prevframe, currentframe, points, sourcepoints):
    ...
    index = 0
    for point in p1:
        if point[0][1] > 1080 or point[0][0] > 1920:
            p1[index] = None

        else:    
            move = flow[point[0][1]][point[0][0]]
            movex = move[0]
            movey = move[1]

            point[0][0] = point[0][0] + movex
            point[0][1] = point[0][1] + movey

        index += 1

    print(sourcepoints[0])
    print(sourcepoints[0][1])
    i = 0
    for point in p1:
        if point == None:
            p1.remove(None)
            sourcepoints = np.delete(sourcepoints[0], i, 0)
        i += 1
    print(sourcepoints)
    # 2) CONSTRUCT HOMOGRAPHY
    img_source = cv2.imread('images/basketballcourt.png')
